...
export default function LeafletMap({ latitude = 13.0827, longitude = 80.2707, onLocationConfirm, readOnly = false, issues = [] }) {
  const mapHtml = `
    ...
    <head>
      ...
      <style>
        ...
        .gmaps-btn:hover { background-color: #f9fafb; }
        ...
        if (issuesData && issuesData.length > 0) {
          ...
          issuesData.forEach(issue => {
            if (issue.latitude && issue.longitude) {
              ...
              const popupContent = '<div style="font-family: sans-serif; padding: 4px; font-size: 12px; line-height: 1.4; min-width: 140px;">' +
  ...
  const handleMessage = (event) => {
    try {
      ...
      if (data && data.latitude && data.longitude && onLocationConfirm) {
  ...
  return (
    <View style={readOnly ? styles.readOnlyContainer : styles.webContainer}>
      <iframe
        srcDoc={mapHtml}
        style={{ width: '100%', height: '100%', border: 'none', borderRadius: 16 }}
        title="SmartFix Leaflet Map"
      />
    </View>
  );
}
